...
int main(){
	...
	if(pid){
		while(running){
			...
			if(msgrcv(msgid,(void *)&rec_data,BUFSIZ,msg_to_receive,0)==-1){
				fprintf(stderr,"msgrcv failed with error : %d\n",errno);
				exit(EXIT_FAILURE);
			}
			printf("Client wrote: %s", rec_data.some_text);
		...
		while(running){
        	...
        	if(msgsnd(msgid2,(void *)&snd_data,BUFSIZ,0)==-1){
